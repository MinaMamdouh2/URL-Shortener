...
// We are gonna path auth as we did for logging for other middlewares, so we have it through closures.
func Authenticate(a *auth.Auth) web.Middleware {
	m := func(handler web.Handler) web.Handler {
		h := func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
			// We call authenticate out of the auth package
			claims, err := a.Authenticate(ctx, r.Header.Get("authorization"))
			if err != nil {
				return auth.NewAuthError("authenticate: failed: %s", err)
			}
			// if we get the claims back, we set them back in the context
...
func Authorize(a *auth.Auth, rule string) web.Middleware {
	m := func(handler web.Handler) web.Handler {
		h := func(ctx context.Context, w http.ResponseWriter, r *http.Request) error {
			// We get the claims out of the context
			...
			// We are parsing the user id of the url
			...
			// Then we call authorize
			if err := a.Authorize(claims, userID, rule); err != nil {
